...
function getActionManga(request, response) {
  const actionManga = mangaDb.filter((manga) =>
...
function getSportsManga(request, response) {
  const sportsManga = mangaDb.filter((manga) =>
    manga.genres.includes("Sports")
  );
  return response.json(sportsManga);
}

function getComedyManga(request, response) {
  const comedyManga = mangaDb.filter((manga) =>
    manga.genres.includes("Comedy")
  );
  return response.json(comedyManga);
}

function getAwardWinning(request, response) {
  const awardWinningManga = mangaDb.filter(
    (manga) => manga.toprated === "true"
  );
